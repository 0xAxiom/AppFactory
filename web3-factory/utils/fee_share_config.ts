 ...
 */
export function validateSocialUsername(provider: SupportedSocialProvider, username: string): void {
  switch (provider) {
    case SupportedSocialProvider.GITHUB:
      // GitHub usernames: alphanumeric, hyphens, max 39 chars
      if (!/^[a-zA-Z0-9-]{1,39}$/.test(username)) {
        throw new Error(`Invalid GitHub username format: ${username}. Must be alphanumeric with hyphens, 1-39 characters.`);
      }
      break;
 ...
 */
export async function resolveSocialFeeClaimer(claimer: SocialFeeClaimer): Promise<WalletFeeClaimer> {
  ...
  try {
    ...
    if (!resolution.success || !resolution.wallet_address) {
      throw new Error(resolution.error_message || `Failed to resolve ${claimer.provider}:${claimer.username}`);
    }
 ...
 */
export async function resolveAllFeeClaimers(feeClaimers: FeeClaimer[]): Promise<WalletFeeClaimer[]> {
...
export function validateFeeClaimersBPS(feeClaimers: FeeClaimer[]): void {
  ...
  if (totalBPS !== 10000) {
    throw new Error(`Fee claimers BPS must total 10000, got ${totalBPS}. Distribute: ${10000 - totalBPS} more BPS.`);
  }
  
  // Validate individual claimer BPS
  for (const claimer of feeClaimers) {
    if (claimer.bps <= 0 || claimer.bps >= 10000) {
      const identifier = 'provider' in claimer ? `${claimer.provider}:${claimer.username}` : claimer.wallet;
      throw new Error(`Invalid BPS for claimer ${identifier}: ${claimer.bps} (must be 1-9999)`);
    }
  }
}
...
export async function createExplicitFeeShareConfig(
  ...
  const additionalBPS = additionalClaimers.reduce((sum, claimer) => sum + claimer.bps, 0);
  const creatorBPS = 10000 - partnerBPS - additionalBPS;
  
  if (creatorBPS <= 0) {
    throw new Error(`Creator BPS would be ${creatorBPS}. Reduce additional claimers or partner BPS.`);
  }
 ...
 */
export async function createSimpleFeeShareConfig(creatorWallet: string): Promise<FeeShareConfiguration> {
...
export async function createOpenSourceFeeShareConfig(
  ...
  const githubClaimers: SocialFeeClaimer[] = githubContributors.map(contributor => ({
...
export class BPSValidationError extends FeeShareConfigError {
  constructor(expectedTotal: number, actualTotal: number, breakdown: Record<string, number>) {
    super(
