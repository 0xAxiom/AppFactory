...
const TEMPLATE_ROOT = join(REPO_ROOT, 'templates', 'mobile');
...
function scaffoldProject(slug) {
  ...
  const templateByType = {
    utility: 'saas-starter',
    social: 'social-app',
    ecommerce: 'e-commerce',
    content: 'saas-starter'
  };

  const templateName = templateByType[state.appType?.value] || 'saas-starter';
  ...
  if (copyTemplate({ templateDir, targetDir: projectPath })) {
    ...
    writeFileSync(
      ...
      JSON.stringify({ template: `templates/mobile/${templateName}` }, null, 2)
    );
