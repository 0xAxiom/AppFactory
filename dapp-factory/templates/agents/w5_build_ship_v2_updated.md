...
function validateTokenLaunchEnvironment(): void {
  const requiredVars = ['BAGS_API_KEY', 'SOLANA_RPC_URL', 'SOLANA_NETWORK', 'CREATOR_WALLET_ADDRESS', 'PRIVATE_KEY'];
...
```typescript
import { resolveSocialFeeClaimer, resolveAllFeeClaimers, validateFeeClaimersBPS } from '../utils/fee_share_config.js';
...
async function executeTokenLaunchV2(w4Config: BagsConfig): Promise<TokenReceipt> {
  ...
  return {
    ...
    feeRouting: {
      ...
      social_claimers:
        w4Config.fee_share_config_v2.social_fee_claimers?.map((claimer) => ({
...
async function createCustomLUTIfRequired(w4Config: BagsConfig): Promise<string | null> {
  ...
  const customLUT = await createLookupTableTransactions(connection, payerKeypair.publicKey, addresses);
...
function isRetryableError(error: Error): boolean {
  ...
  const retryablePatterns = ['rate limit', 'network error', 'timeout', 'server error', '429', '500', '502', '503'];
...
async function handleSocialProviderErrors(w4Config: BagsConfig): Promise<WalletFeeClaimer[]> {
  ...
  if (failures.length > 0) {
    ...
    // Note: When using skip_unresolved strategy, remaining BPS goes to creator
    // Implementation of BPS redistribution is handled by the Bags SDK
  }
