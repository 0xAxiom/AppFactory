...
export async function lookupZoraProfile(
  identifier: string
...
export async function buyZoraCoin(
  ...
  try {
    if (!profile.creatorCoin?.address) {
      return {
        success: false,
        error: 'No creator coin found for this profile',
      };
    }
    ...
    console.log(
      `Simulating purchase of ${amountUSD} USD worth of coins for ${profile.handle}`
    );
