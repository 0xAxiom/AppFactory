  ...
  try {
    ...
    if (!zoraHandle || !amountUSD || !spendCalls) {
      return NextResponse.json(
        { error: 'Missing zoraHandle, amountUSD, or spendCalls' },
        { status: 400 }
      );
    }
    ...
    if (!serverWallet) {
      return NextResponse.json(
        {
          error:
            'Server wallet not found in memory (possibly due to server restart). Please re-setup your spend permissions.',
        },
        { status: 400 }
      );
    }
    ...
    console.log(
      `Processing Zora coin purchase: $${amountUSD} for ${zoraHandle}`
    );
    ...
    if (!serverWallet.smartAccount) {
      return NextResponse.json(
        {
          error:
            'Smart account not found. Please re-setup your spend permissions.',
        },
        { status: 400 }
      );
    }
    ...
    if (!profile) {
      return NextResponse.json(
        { error: `Zora profile not found for handle: ${zoraHandle}` },
        { status: 404 }
      );
    }
    ...
    if (!profile.creatorCoin) {
      return NextResponse.json(
        { error: `No creator coin found for ${profile.handle}` },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      try {
        ...
        if (creatorCoinBalance > 0) {
          ...
          const transferAmount = (creatorCoinBalance as bigint)
            .toString(16)
            .padStart(64, '0');
        ...
        console.error(
          'Failed to transfer creator coins to user:',
          transferError
        );
      }
    } catch (error) {
    ...
    return NextResponse.json(
      { error: 'Failed to purchase Zora coin' },
      { status: 500 }
    );
  }
}
