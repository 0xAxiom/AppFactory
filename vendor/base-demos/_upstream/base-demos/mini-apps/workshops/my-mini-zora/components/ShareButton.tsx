...
export function ShareButton({ displayName, tokens = [] }: ShareButtonProps) {
  ...
  const waitForVisualRender = async (): Promise<boolean> => {
    ...
    while (Date.now() - startTime < timeout) {
      ...
      const allImagesVisible = Array.from(images).every((img) => {
        ...
        const isVisuallyRendered =
          isLoaded && hasVisibleDimensions && notHidden;
    ...
    console.warn(
      "⚠️ [SHARE-BUTTON] Timeout - some images may not be visually rendered",
    );
    return false;
  };
  ...
  const generateOGImage = async () => {
    ...
    console.log(
      "🎨 [SHARE-BUTTON] Generating OG image for:",
      displayName,
      "with",
      tokens.length,
      "tokens",
  ...
  const captureAndSaveImage = async () => {
    ...
    console.log(
      "📸 [SHARE-BUTTON] Starting capture process for displayName:",
      displayName,
    );
    ...
    console.log(
      "✅ [SHARE-BUTTON] Found collage container:",
      collageContainer.offsetWidth + "x" + collageContainer.offsetHeight,
    );
    ...
    try {
      ...
      console.log(
        "⏳ [SHARE-BUTTON] Waiting for images to be visually rendered...",
      );
      const imagesRendered = await waitForVisualRender();

      if (!imagesRendered) {
        console.warn(
          "⚠️ [SHARE-BUTTON] Proceeding with capture despite some images not being fully rendered",
        );
      }
      ...
      const nextImageDivs = collageContainer.querySelectorAll(
        '[style*="position: relative"]',
      );
      console.log(
        "🖼️ [SHARE-BUTTON] Next.js Image containers found:",
        nextImageDivs.length,
      );
      ...
      console.log(
        "✅ [SHARE-BUTTON] Successfully captured image:",
        dataUrlType,
        "Size:",
        dataUrlSize,
        "chars",
      ...
      console.log(
        "✅ [SHARE-BUTTON] Successfully uploaded! Blob URL:",
        blobUrl,
      );
      ...
      console.error(
        "❌ [SHARE-BUTTON] Failed to capture and save image:",
        error,
      );
      setStatus("error");
      setErrorMessage(
  ...
  const getButtonText = () => {
    switch (status) {
      case "capturing":
        return "Loading images & capturing...";
      case "uploading":
        return "Uploading...";
      case "ready":
        return "Share to Base App";
      case "sharing":
        return "Opening Base App...";
      case "error":
        return "Try again";
      default:
        return "Create Shareable Image";
    }
  };
