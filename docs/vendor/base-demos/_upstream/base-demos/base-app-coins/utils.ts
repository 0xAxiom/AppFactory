...
export async function loadData(key: PoolKey) {
  ...
  const [sqrtPriceX96, tick, _protocolFee, _lpFee] =
...
export async function categorizeAppType(pool: Pool) {
  ...
  const [currency0PlatformReferrer, currency1PlatformReferrer] =
    await Promise.all([
  ...
  if (
    [currency0PlatformReferrer, currency1PlatformReferrer].includes(
