...
export async function loadData(key: PoolKey) {
  const [currency0, currency1] = await Promise.all([getCurrency(key.currency0), getCurrency(key.currency1)]);
...
export async function getCurrency(address: string): Promise<Currency> {
  ...
  const [name, symbol, decimals] = await Promise.all([erc20.read.name(), erc20.read.symbol(), erc20.read.decimals()]);
...
export async function categorizeAppType(pool: Pool) {
  async function tryGetPlatformReferrer(address: string) {
    const zoraBaseCoin = getContract({
      abi: parseAbi(['function platformReferrer() view returns (address)']),
      address: address as `0x${string}`,
      client: publicClient,
    });
