...
const serverWallets =
  globalThis.__serverWallets ?? new Map<string, ServerWallet>();
globalThis.__serverWallets = serverWallets;

export async function createServerWalletForUser(
  userAddress: string
  ...
  try {
    ...
    if (
      serverWallets.has(userAddress) &&
      serverWallets.get(userAddress)?.smartAccount
    ) {
    ...
    console.log(
      'Stored server wallet with smart account. New count:',
      serverWallets.size
    );
...
export function getServerWalletForUser(
  userAddress: string
): ServerWallet | null {
