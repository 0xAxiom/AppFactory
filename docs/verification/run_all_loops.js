...
const catalog = JSON.parse(
  fs.readFileSync('plugin-factory/mcp.catalog.json', 'utf8')
);
...
function loop1() {
  ...
  checks.push({
    name: 'Pipeline mappings complete',
    pass: pipelineMappingsComplete,
  });

  // Check 4: Phase mappings consistent
  const validPhases = ['research', 'build', 'verify', 'deploy', 'ralph'];
  let phaseMappingsConsistent = true;
  for (const [pipeline, config] of Object.entries(
    catalog.pipelineIntegrations
  )) {
  ...
  checks.push({
    name: 'Phase mappings consistent',
    pass: phaseMappingsConsistent,
  });

  // Check 5: No duplicate MCP definitions
  const mcpIds = Object.keys(catalog.mcpServers);
  checks.push({
    name: 'No duplicate MCP definitions',
    pass: mcpIds.length === new Set(mcpIds).size,
  });
  ...
  return {
    loop: 1,
    name: 'Catalog Structure Validation',
...
function loop2() {
  ...
  for (const test of testCases) {
    ...
    log(
      `  ${test.mcp} in ${test.wrongPhase}: ${blocked ? 'PASS (blocked)' : 'FAIL'}`
    );
    if (!blocked) {
      allPassed = false;
      results.issues.push(
        `${test.mcp} not blocked in ${test.wrongPhase} phase for dapp-factory`
      );
    }
  }
  ...
  return {
    loop: 2,
    name: 'Phase Gate Enforcement (dapp-factory)',
...
function loop3() {
  ...
  for (const test of testCases) {
    ...
    if (!blocked) {
      allPassed = false;
      results.issues.push(
        `${test.mcp} not blocked in ${test.wrongPhase} phase for app-factory`
      );
    }
  }
  ...
  return {
    loop: 3,
    name: 'Phase Gate Enforcement (app-factory)',
    status: allPassed ? 'PASS' : 'FAIL',
  };
}
...
function loop4() {
  ...
  const pipelineTests = [
    {
      ...
      tests: [
        { mcp: 'supabase', wrongPhase: 'ralph' },
        { mcp: 'cloudflare', wrongPhase: 'build' },
      ],
    },
    {
  ...
  for (const pt of pipelineTests) {
    ...
    for (const test of pt.tests) {
      ...
      const blocked = !allowedPhases.includes(test.wrongPhase);
      log(
        `  ${pt.pipeline}/${test.mcp} in ${test.wrongPhase}: ${blocked ? 'PASS' : 'FAIL'}`
      );
  ...
  return {
    loop: 4,
    name: 'Phase Gate Enforcement (Other Pipelines)',
    status: allPassed ? 'PASS' : 'FAIL',
  };
}
...
function loop5() {
  ...
  for (const test of permissionTests) {
    ...
    log(
      `  ${test.mcp}: ${actualLevel} - ${isCorrect ? 'PASS' : 'FAIL (expected ' + test.level + ')'}`
    );
  ...
  for (const mcpId of mutatingMcps) {
    ...
    if (mcp.permissionLevel !== 'read-only') {
      ...
      log(
        `  ${mcpId} approval required: ${hasApproval ? 'PASS' : 'WARN (should require approval)'}`
      );
    }
  }
  ...
  return {
    loop: 5,
    name: 'Permission Level Enforcement',
    status: allPassed ? 'PASS' : 'FAIL',
  };
}
...
function loop6() {
  ...
  for (const test of envTests) {
    const mcp = catalog.mcpServers[test.mcp];
    const hasEnvVar =
      mcp.requiredEnvVars && mcp.requiredEnvVars.includes(test.envVar);
    const hasFailureBehavior =
      mcp.failureBehavior && mcp.failureBehavior.onAuthError;
    log(
      `  ${test.mcp}: requires ${test.envVar} - ${hasEnvVar ? 'PASS' : 'FAIL'}`
    );
    log(
      `    failure behavior defined: ${hasFailureBehavior ? 'PASS' : 'WARN'}`
    );
...
function loop7() {
  ...
  for (const test of artifactTests) {
    ...
    const catalogArtifacts =
      catalog.artifactOutputs && catalog.artifactOutputs[test.mcp];
  ...
  return {
    loop: 7,
    name: 'Artifact Output Verification',
    status: allPassed ? 'PASS' : 'FAIL',
  };
}
...
function loop8() {
  ...
  for (const [key, pipelines] of Object.entries(pipelineMcpUsage)) {
    if (pipelines.length > 1) {
      log(
        `  ${key} used by: ${pipelines.join(', ')} - OK (isolated by pipeline)`
      );
    }
  }
  ...
  const hasGlobalRules =
    catalog.globalRules && Object.keys(catalog.globalRules).length > 0;
  log(`  Global isolation rules defined: ${hasGlobalRules ? 'PASS' : 'FAIL'}`);

  log(
    `Loop 8 Status: ${conflicts === 0 && hasGlobalRules ? 'PASS' : 'NEEDS FIX'}\n`
  );
...
function loop9() {
  ...
  for (const pipeline of pipelines) {
    ...
    const hasPhaseMapping =
      config.phaseMapping && Object.keys(config.phaseMapping).length > 0;

    log(
      `  ${pipeline}: MCPs defined - ${hasAllowedMcps ? 'PASS' : 'WARN'}, phases mapped - ${hasPhaseMapping ? 'PASS' : 'WARN'}`
    );
  }

  // Check that failure handling doesn't break determinism
  const hasSafeDefaults = Object.values(catalog.mcpServers).every(
    (mcp) =>
      mcp.safeModeDefaults && Object.keys(mcp.safeModeDefaults).length > 0
  );
  log(
    `  Safe mode defaults for all MCPs: ${hasSafeDefaults ? 'PASS' : 'WARN'}`
  );

  log(`Loop 9 Status: ${allPassed ? 'PASS' : 'NEEDS FIX'}\n`);
  return {
    loop: 9,
    name: 'Deterministic Pipeline Completion',
    status: allPassed ? 'PASS' : 'FAIL',
  };
}
...
function loop10() {
  ...
  for (const [mcpId, mcp] of Object.entries(catalog.mcpServers)) {
    const hasFailureBehavior =
      mcp.failureBehavior && Object.keys(mcp.failureBehavior).length > 0;
    log(
      `  ${mcpId}: failure behavior defined - ${hasFailureBehavior ? 'PASS' : 'FAIL'}`
    );
    if (!hasFailureBehavior) allPassed = false;
  }

  // Check global rules prevent silent failures
  const preventsSilentFailures =
    catalog.globalRules &&
    (catalog.globalRules.artifactLogging ||
      catalog.globalRules.failureHandling);
  log(
    `  Global failure logging rules: ${preventsSilentFailures ? 'PASS' : 'FAIL'}`
  );

  log(
    `Loop 10 Status: ${allPassed && preventsSilentFailures ? 'PASS' : 'NEEDS FIX'}\n`
  );
  return {
    loop: 10,
    name: 'Silent Failure Detection',
    status: allPassed && preventsSilentFailures ? 'PASS' : 'FAIL',
  };
}
...
function loop11() {
  ...
  const hasGovernance =
    catalog.mcpGovernance && typeof catalog.mcpGovernance === 'object';
  ...
  const hasSpecUrl =
    catalog.mcpGovernance && catalog.mcpGovernance.specificationUrl;
  ...
  const hasClarification =
    catalog.mcpGovernance &&
    catalog.mcpGovernance.clarification &&
    catalog.mcpGovernance.clarification.MCP &&
    catalog.mcpGovernance.clarification['MCP Server'];
  checks.push({
    name: 'MCP vs MCP Server distinction documented',
    pass: hasClarification,
  });
  log(
    `  MCP vs MCP Server distinction documented: ${hasClarification ? 'PASS' : 'FAIL'}`
  );

  // Check 4: Governance note explains MCP is not executable
  const hasGovernanceNote =
    catalog.mcpGovernance &&
    catalog.mcpGovernance.governanceNote &&
    catalog.mcpGovernance.governanceNote.toLowerCase().includes('not a tool');
  checks.push({
    name: 'Governance note explains MCP not executable',
    pass: hasGovernanceNote,
  });
  log(
    `  Governance note explains MCP not executable: ${hasGovernanceNote ? 'PASS' : 'FAIL'}`
  );
  ...
  return {
    loop: 11,
    name: 'MCP Governance Compliance',
    status: allPassed ? 'PASS' : 'FAIL',
    checks,
  };
}
