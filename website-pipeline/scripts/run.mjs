...
const TEMPLATE_ROOT = join(REPO_ROOT, 'templates', 'website');
...
function scaffoldProject(slug) {
  ...
  const templateName = templateByType[state.siteType?.value] || 'portfolio';
  ...
  if (copyTemplate({ templateDir, targetDir: projectPath })) {
    ...
    writeFileSync(
      ...
      JSON.stringify({ template: `templates/website/${templateName}` }, null, 2)
    );
    ...
    if (hasReactSkills) {
      ...
      try {
        ...
        if (existsSync(auditScript)) {
          ...
          execSync(`bash "${auditScript}" "${projectPath}" --skill web-design-guidelines`, {
            stdio: 'inherit'
          });
          console.log(`${GREEN}✓ Skills audits passed${RESET}\n`);
        } else {
...
async function main() {
  ...
  try {
    ...
    printCapabilityReport(capabilities, {
      detailed: true,
      showAll: false // Only show unavailable in summary
    });
  } catch (err) {
      ...
      if (hasRalph) {
        ...
        if (existsSync(ralphScript)) {
          ...
          if (answer.toLowerCase() === 'y' || answer.toLowerCase() === 'yes') {
            ...
            execSync(`bash "${ralphScript}" website-pipeline 1`, { stdio: 'inherit' });
            console.log(`\n${GREEN}✓ Ralph QA iteration 1 complete${RESET}`);
            console.log(`${DIM}Continue manually: ./ralph/run-ralph.sh website-pipeline 2${RESET}\n`);
          } else {
        ...
        console.log(`${DIM}Manual: ./ralph/run-ralph.sh website-pipeline 1${RESET}\n`);
      }
    } catch (err) {
