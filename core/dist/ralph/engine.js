...
export class RalphEngine {
    ...
    async run() {
        ...
        this.logger.info(`Starting Ralph QA (threshold: ${this.config.passingThreshold}%, max iterations: ${this.config.maxIterations})`);
        let iteration = 0;
        ...
        while (iteration < this.config.maxIterations) {
            ...
            const result = await this.runIteration(iteration, iterationResults[iterationResults.length - 1]);
        ...
        if (finalVerdict === 'PASS') {
            ...
            this.logger.success(`Ralph PASS - All criteria met (score: ${lastResult.score}%)`);
        }
        else {
            this.logger.error(`Ralph FAIL - Score ${lastResult.score}% below threshold ${this.config.passingThreshold}%`);
        }
        return report;
    }
    /**
    ...
    async runIteration(iteration, previousResult) {
        ...
        // Calculate score
        const { score, issuesToFix } = this.calculateScore(checkResults, skillAudits, e2eResults);
        // Determine verdict
    ...
    async runSkillAudits() {
        ...
        for (const skillConfig of this.config.skills) {
            if (this.options.runSkillAudit) {
                const result = await this.options.runSkillAudit(skillConfig.name, this.buildPath);
                results.push(result);
            }
            else {
    ...
    calculateScore(checkResults, skillAudits, e2eResults) {
        ...
        issuesToFix.sort((a, b) => severityOrder[a.severity] - severityOrder[b.severity]);
