...
export function loadPipelineConfig(
  configPath: string
): ConfigLoadResult<PipelineConfig> {
  ...
  return {
    success: true,
    config: result.data as PipelineConfig,
  };
}
...
export function loadRalphConfig(
  ...
  return {
    success: true,
    config: result.data as RalphConfig,
  };
}
...
export function loadValidationConfig(
  configPath: string
): ConfigLoadResult<ValidationConfig> {
  if (!exists(configPath)) {
    return {
      success: false,
      errors: [`Configuration file not found: ${configPath}`],
  ...
  return {
    success: true,
    config: result.data as unknown as ValidationConfig,
  };
}
 ...
 * @returns Configuration load result
 */
export function loadPipelineConfigFile(
  configPath: string
): ConfigLoadResult<PipelineConfigFile> {
  if (!exists(configPath)) {
    return {
      success: false,
      errors: [`Configuration file not found: ${configPath}`],
  ...
  return {
    success: true,
    config: result.data as unknown as PipelineConfigFile,
  };
}
 ...
 * @returns Configuration load result
 */
export async function discoverPipelineConfig(
  pipelineDir: string
): Promise<ConfigLoadResult<PipelineConfigFile>> {
  // Try JSON first
