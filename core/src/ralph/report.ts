...
import type { Issue } from '../types/pipeline.js';

/**
 * Generate the PROGRESS.md content
 */
export function generateProgressMarkdown(report: RalphReport): string {
  const lines: string[] = [];
  ...
  for (const iteration of report.iterationResults) {
    ...
    if (iteration.e2eResults) {
      ...
      lines.push(
        `**E2E Tests:** ${status} (${e2e.passedCount}/${e2e.total} passed)`
      );
      lines.push('');
    }
 ...
 */
export function generateLoopMarkdown(report: RalphReport): string {
  const lines: string[] = [];
  ...
  for (const iteration of report.iterationResults) {
    ...
    lines.push(
      `| ${iteration.iteration} | ${iteration.score}% | ${iteration.verdict} | ${(iteration.duration / 1000).toFixed(1)}s | ${issuesFixedStr} |`
    );
 ...
 */
export function generateQANotesMarkdown(report: RalphReport): string {
  const lines: string[] = [];
  ...
  // Strengths
  const lastResult =
    report.iterationResults[report.iterationResults.length - 1];
 ...
 */
export function generateAcceptanceMarkdown(
 ...
 */
export function parseProgressEntry(
  markdown: string,
  iteration: number
): RalphProgressEntry | null {
  const iterationRegex = new RegExp(
    `### Iteration ${iteration}[\\s\\S]*?(?=###|$)`
  );
