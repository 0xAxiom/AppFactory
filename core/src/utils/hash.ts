 ...
 */
export function generateRunId(pipeline: string, suffix?: string): string {
 ...
 */
export function generateContentAddressedRunId(
  pipeline: string,
  intent: string
): string {
 ...
 */
export function slugify(name: string, maxLength: number = 30): string {
  return name
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, '-')
    .replace(/^-+|-+$/g, '')
