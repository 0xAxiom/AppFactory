...
async function main() {
  ...
  process.on('SIGINT', () => {
    console.error('decision-memory: Shutting down...');
    process.exit(0);
  });
  ...
  console.error('decision-memory MCP server running on stdio');
  console.error('Database: ~/.decision-memory/decisions.db');
}

main().catch((error) => {
  console.error('Failed to start decision-memory server:', error);
  process.exit(1);
});
